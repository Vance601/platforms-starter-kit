      ...
      {claimedTruck ? (
        batteries.length === 0 ? (
          ...
          <div className="space-y-2">
            <p className="text-sm text-slate-400">Tap the battery you installed.</p>
            {batteries.map((b) => {
              ...
              return (
                ...
                >
                  ...
                  {b.serial_number ? (
                    <p className="text-xs text-slate-500">S/N {b.serial_number}</p>
                  ) : null}
                </button>
              );
            })}
        ...
        <div className="rounded-lg border border-slate-700 bg-slate-800/40 px-4 py-3">
          <label className="flex items-center gap-3">
            <input
              ...
              className="h-5 w-5"
            />
            <span className="font-medium">Warranty replacement (FREE — $0)</span>
          </label>
              ...
              >
                ...
                {soldBatteries.map((sb) => (
                  <option key={sb.id} value={sb.id}>
                    {sb.barcode}
                    {sb.sold_on_call_number ? ` (call #${sb.sold_on_call_number})` : ""}
                  </option>
                ))}
