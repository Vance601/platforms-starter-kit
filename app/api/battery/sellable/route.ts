...
export async function GET(req: NextRequest) {
  try {
    ...
    // The truck this driver currently has claimed.
    ...
    // Batteries currently ON this driver's truck (status on_truck, truck_id = their truck).
    let batteries: Record<string, unknown>[] = [];
    if (claimedTruck) {
      const { rows } = await sql`
        SELECT id, barcode, serial_number, status
        FROM batteries
        WHERE status = 'on_truck'
          AND truck_id = ${claimedTruck.id}
