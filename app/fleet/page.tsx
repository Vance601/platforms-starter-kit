...
import { PlusCircle, Truck as TruckIcon, Battery, RotateCcw, User } from "lucide-react"
import {
  Dialog,
  DialogContent,
  DialogDescription,
...
export default function FleetPage() {
  ...
  function loadTrucks() {
    setIsLoading(true)
    fetch("/api/trucks/overview", { cache: "no-store" })
      .then((r) => r.json())
      .then((j) => {
        if (j?.success && Array.isArray(j.trucks)) setTrucks(j.trucks)
      })
      .catch(() => {})
    ...
    <div className="flex flex-col gap-6">
      ...
      {isLoading ? (
        <div className="flex items-center justify-center py-24">
          <p className="text-lg text-muted-foreground">Loading live truck data…</p>
        </div>
      ) : (
        <>
          {/* Top stat cards — live from the database */}
