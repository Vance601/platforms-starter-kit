...
// Add the transferInventory function to the interface
interface InventoryState {
  ...
  addTruck: (truck: any) => void // Added this function
...
// Helper function to check if we need to initialize with sample data
const shouldInitializeWithSampleData = () => {
  try {
    const storedState = localStorage.getItem("inventory-storage")
    if (!storedState) return true

    const parsedState = JSON.parse(storedState)
    return !parsedState.state || !parsedState.state.trucks || parsedState.state.trucks.length === 0
  } catch (error) {
...
// Create a debounced storage function to prevent excessive writes
...
export const useInventoryStore = create<InventoryState>()(
  persist(
    (set, get) => ({
      ...
      // Add the addTruck function
      addTruck: (truck) => {
        console.log("Inventory store: Adding truck", truck)

        set((state) => {
          // Ensure the truck has an ID if not provided
          const truckWithId = truck.id
            ? truck
            : {
                ...truck,
                id: `TRK-${Date.now()}`,
          ...
          // Create a new trucks array with the new truck added
          const updatedTrucks = Array.isArray(state.trucks) ? [...state.trucks, truckWithId] : [truckWithId]

          console.log("Adding truck:", truckWithId)
          console.log("Updated trucks array:", updatedTrucks)

          // Force a storage update by creating a new state object
          return {
            ...state,
        ...
        // Verify the truck was added by logging the current state
        const currentTrucks = get().trucks
        console.log("Current trucks after adding:", currentTrucks)
      },

      receiveInventory: (items, invoiceNumber) =>
        set((state) => {
      ...
      verifyInventory: (itemId, userId) =>
        set((state) => {
          // Ensure pendingInventory and holdInventory are arrays
          const pendingInventory = Array.isArray(state.pendingInventory) ? state.pendingInventory : []
          const holdInventory = Array.isArray(state.holdInventory) ? state.holdInventory : []

          // Find the pending item in either array
          const pendingItem =
            pendingInventory.find((item) => item.id === itemId) || holdInventory.find((item) => item.id === itemId)
          ...
          // Filter out the verified item from pendingInventory and holdInventory
          const updatedPendingInventory = pendingInventory.filter((item) => item.id !== itemId)
          const updatedHoldInventory = holdInventory.filter((item) => item.id !== itemId)

          // Ensure verifiedInventory is an array
          const verifiedInventory = Array.isArray(state.verifiedInventory) ? state.verifiedInventory : []

          // Ensure inventoryHistory is an array
          const inventoryHistory = Array.isArray(state.inventoryHistory) ? state.inventoryHistory : []

          return {
            inventory: updatedInventory,
            pendingInventory: updatedPendingInventory,
            holdInventory: updatedHoldInventory,
            verifiedInventory: [...verifiedInventory, verifiedItem],
            inventoryHistory: [...inventoryHistory, historyItem],
          }
        }),
      ...
      assignBatteryToTruck: (truckId, batteryDetails, quantity, userId, notes) =>
        set((state) => {
      ...
      transferInventory: (transferData) =>
        set((state) => {
          if (!Array.isArray(state.inventory)) {
            console.error("transferInventory: inventory is not an array")
            return state
          }

          const { type, model, sourceLocation, destinationLocation, quantity, notes } = transferData

          // Find the inventory item
          const inventoryItemIndex = state.inventory.findIndex((item) => item.type === type && item.model === model)

          if (inventoryItemIndex === -1) {
            console.error(`transferInventory: item not found - ${type} ${model}`)
            return state
          }
          ...
          const sourceKey = sourceLocation.toLowerCase() as "broadway" | "camelback"
          const destKey = destinationLocation.toLowerCase() as "broadway" | "camelback"

          const sourceQuantity = inventoryItem.locations[sourceKey] || 0

          if (sourceQuantity < quantity) {
            console.error(
              `transferInventory: not enough inventory at source location. Available: ${sourceQuantity}, Requested: ${quantity}`,
            )
            return state
          }
          ...
          const updatedLocations = { ...inventoryItem.locations }

          // Decrease from source
          updatedLocations[sourceKey] = Math.max(0, sourceQuantity - quantity)

          // Increase at destination
          updatedLocations[destKey] = (updatedLocations[destKey] || 0) + quantity
